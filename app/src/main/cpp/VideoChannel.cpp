...
void VideoChannel::initVideoEncoder(int width, int height, int fps, int bitrate) {
    ...
    if (videoEncoder) {
        x264_encoder_close(videoEncoder);  // 防止重复初始化
    ...
    // 告诉x264，到底是什么时候，来一个I帧， 计算关键帧的距离
    // 帧距离(关键帧)  2s一个关键帧   （就是把两秒钟一个关键帧告诉x264）
...
void VideoChannel::encodeData(signed char *data) {
    ...
    if (ret < 0) { // 返回值：x264_encoder_encode函数 返回的 NAL 中的字节数。如果没有返回 NAL 单元，则在错误时返回负数和零。
        LOGE("x264编码失败");
    ...
    // 发送 Packets 入队queue
    // sps(序列参数集) pps(图像参数集) 就是：告诉ffmpeg如何解码图像数据
    ...
    for (int i = 0; i < pi_nal; ++i) {
        if (nal[i].i_type == NAL_SPS) {
            sps_len = nal[i].i_payload - 4; // 去掉起始码（00 00 00 01）
            memcpy(sps, nal[i].p_payload + 4, sps_len); // 由于上面减了4，所以+4挪动这里的位置开始
        } else if (nal[i].i_type == NAL_PPS) {
            pps_len = nal[i].i_payload - 4; // 去掉起始码 (00 00 00 01）
...
void VideoChannel::sendSpsPps(uint8_t *sps, uint8_t *pps, int sps_len, int pps_len) {
    // TODO RTMP包格式
    //     关键帧：0x17 01 00 00 00 {四个字节数据长度} {h264数据}
    //   非关键帧：0x27 01 00 00 00 {四个字节数据长度} {h264数据}
    //   sps与pps：0x17 00 00 00 00 {sps + pps数据}

    // sps 序列参数集  pps 图像参数集

    // 5 个字节 代表帧类型
    //     关键帧：0x17 01 00 00 00
    //   非关键帧：0x27 01 00 00 00
    //   sps与pps：0x17 00 00 00 00
    // 8个字节代表
    //      1字节：版本
    //      2字节：sps[1] profile
    //      3字节：sps[2] 兼容性
    //      4字节：sps[3] profile level
    //      5字节：包长度   通常：0xFF
    //      6字节：sps个数  通常：0xE1
    //    7-8字节：sps长度
    // 3个字节代表
    ...
    // packet 存入队列,通过函数指针把数据回调到native-lib.cpp中存入队列
